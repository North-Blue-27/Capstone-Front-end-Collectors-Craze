...
function Header() {
  ...
  const handleRegister = async () => {
    try {
      ...
      const response = await axios.post('http://localhost:3001/register', registrationData); // Effettua una richiesta POST per la registrazione
      ...
      <Navbar className='navbar-style bg-dark' variant="dark" expand="lg">
        ...
        <Navbar.Collapse id="responsive-navbar-nav">
          <Form className="d-md-flex align-items-center">
            ...
            {isLoggedIn ? (
              <Button variant="outline-light" className="mr-2" onClick={handleLogout}>
                Logout
              </Button>
            ) : (
              <>
